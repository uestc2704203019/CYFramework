...
#define mainName @"CYMainViewController"
...
{
    SliderViewController *sharedSVC = [[SliderViewController alloc] init];
    return sharedSVC;
...
- (void)viewDidLoad
{
    ...
    [self initChildControllers:_LeftVC rightVC:_RightVC mainVC:_MainVC];
    
//    [self showContentControllerWithModel:_MainVC!=nil?NSStringFromClass([_MainVC class]):mainName];
    ...
    _tapGestureRec = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(tapGestureAction:)];
...
- (void)setHasBounces:(BOOL)hasBounces
{
    _hasBounces = hasBounces;
}
...
{
    if (![tapGesture.view isEqual:_mainContentView]) {
        return;
    }
    [self closeSideBar];
}
...
- (void)moveViewWithGesture:(UIPanGestureRecognizer *)panGes
{
    ...
    if (panGes.state == UIGestureRecognizerStateChanged)
    {
        ...
        if (transX > 0)
        {
            ...
            [self.view sendSubviewToBack:_rightSideView];
            [self configureViewShadowWithDirection:RMoveDirectionRight];
            if (!_hasBounces && _LeftStartX+transX>=_LeftSContentOffset) {
                return;
            }
            _rightSideView.frame=CGRectMake(_RightStartX, 0, _rightSideView.frame.size.width, _rightSideView.frame.size.height);
        ...
        else    //transX < 0
        {
            ...
            [self.view sendSubviewToBack:_leftSideView];
            [self configureViewShadowWithDirection:RMoveDirectionLeft];
            if (!_hasBounces && fabs(_RightStartX+transX)>=_RightSContentOffset) {
                return;
            }
...
{
    CGPoint touchPoint = [touch locationInView:_mainContentView];
    if (showingRight) {
        if (touchPoint.x>CYScreenWidth||touchPoint.y>CYScreenHeight) {
            return NO;
        }
    }
    ...
    if ([NSStringFromClass([touch.view class]) isEqualToString:@"CYRightViewController"]) {
        return NO;
    }
    return  YES;
}
